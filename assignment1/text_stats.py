...
def plot_word_distributions(top_words):
    ranks = range(1, len(top_words) + 1)
    # another way to write frequencies = [freq for _, freq in top_words]
    # create a new list we want
    # iterate through top words
    # unpack the top words as tuple, since we do not want the first part underscore is used
    # freq is what we want
    # appened the freq to frequencies
    frequencies = []
...
topwords = word_frequencies_withstop(data)
plot_word_distributions(topwords)